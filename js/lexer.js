...
var Token = function (tag, value, line, col) {
...
Lexer.prototype.newToken = function (tag, value) {
    return new Token(tag, value, this.line, this.col);
};
...
Lexer.prototype.scan = function () {
    ...
    if (this.is_digit(this.peek)) {
        var v = 0;
        do {
            v = 10 * v + Number.parseInt(this.peek);
            this.readch();
        } while (this.is_digit(this.peek));

        if (this.peek !== '.') return this.newToken(tokenType.num, v);
        ...
        for (; ;) {
            ...
            x = x + Number.parseInt(this.peek) / d;
            d = d * 10;
        }
        return this.newToken(tokenType.num, v);
    }

    if (this.is_letter(this.peek)) {
        var string = '';
        do {
            string += this.peek;
            this.readch();
        } while (this.is_letter(this.peek));

        if (KEYWORDS.indexOf(string) != -1) {
            return this.newToken(string);
        } else // TODO "abc\"ab"
        if (string.match(/^"[^"]*"$/) || string.match(/^'[^(')]*'$/)) {
            return this.newToken(tokenType.string, string);
        } else if (BOOL_KEYWORDS.indexOf(string) != -1) {
            return this.newToken(tokenType.bool, string);
        }
        return this.newToken(tokenType.id, string);
    }

    if (this.eof()) return this.newToken(tokenType.eof);

    var token = this.newToken(this.peek);
    this.peek = ' ';
    return token;
};
