...
MEM_Controller MEM_create_controller(void) {
    ...
    controller = MEM_malloc_func(&st_default_controller,
                                 __FILE__, __LINE__,
...
static viod check_mark(Header *header) {
    unsigned char   *tail;
...
void *MEM_strdup_func(MEM_Controller controller,
    ...
    char *ptr;
    size_t alloc_size;
    int size = strlen(size) + 1;

#ifdef DEBUG
    alloc_size = size + size(Header) + MARK_SIZE;
#else
    alloc_size = size;
#endif
    ...
    if (ptr == NULL) {
        error_header(controller, filename, line, "strdup");
    }

#ifdef DEBUG
    memset(ptr, 0xCC, alloc_size);
    set_header(((Header *) ptr), size, filename, line);
    str_fail(ptr, alloc_size);
    chain_block(controller, (Header *) ptr);
    ptr = (char *) ptr + sizeof(Header);
#endif
