...
DBG_Controller DBG_create_controller_func(void) {
    ...
    controller->debug_wirte_fp = NULL;
    controller->current_debug_level = INT_MAX;

    return controller;
}

void DBG_set_debug_level_func(DBG_Controller controller,
                              int level) {
    controller->current_debug_level = level;
}

void DBG_set_debug_write_fp_func(DBG_Controller controller,
                                 FILE *fp) {
...
void DBG_assert_func(char *fmt, ...) {
    ...
    assert_func(stderr, st_current_file_name,
                st_current_line, st_assert_expression,
                fmt, ap);
    va_end(ap);
    abort();
}
...
void DBG_panic_func(char *fmt, ...) {
    ...
    panic_func(st_current_controller.debug_write_fp,
               st_current_file_name, st_current_line,
               fmt, ap);
    panic_func(stderr, st_current_file_name,
               st_current_line, fmt, ap);
    va_end(ap);
    abort();
}
...
void DBG_debug_write_func(int level, char *fmt, ...) {
    va_list ap;
    if (level > 0 && level > st_current_controller->current_debug_leval) {
        return;
    }
    va_start(ap, fmt);
    initialize_debug_write_fp();
